%namespace Microsoft.Pc.Parser
%using Microsoft.Pc.Domains;
%visibility internal 
%YYSTYPE LexValue
%sharetokens
%partial
%tokentype PTokens
%parsertype DummyTokenParser

%token INT BOOL FLOAT ANY SEQ MAP ID
%token TYPE INCLUDE MAIN EVENT MACHINE ASSUME SPEC ENUM EVENTSET DATA INTERFACE
%token QMACHINE TOPIC

%token VAR START HOT COLD STATE FUN ACTION GROUP OBSERVES

%token ENTRY EXIT DEFER IGNORE GOTO ON DO PUSH AS WITH

%token ATLEASTNEW ATMOSTLOSE
%token PUBLISH READ
%token PUBLISHES SUBSCRIBES
%token PERIOD

%token IF WHILE THIS NEW RETURN ID POP ASSERT PRINT CALL RAISE SEND DEFAULT HALT NULL RECEIVE CASE ANNOUNCE
%token LPAREN RPAREN LCBRACE RCBRACE LBRACKET RBRACKET SIZEOF KEYS VALUES

%token TRUE FALSE

%token EVENTSET
%token RECEIVES SENDS CREATES

%token SWAP, MOVE

%token ASSIGN REMOVE INSERT
%token EQ NE LT GT LE GE IN
%left LAND LNOT LOR NONDET FAIRNONDET

%token DOT COLON COMMA
%left  SEMICOLON

%token INT BOOL STR

%left  PLUS MINUS
%left  DIV
%left  MUL 
%token ELSE

%token MODULE COMPOSE HIDEE EXPORT SAFE RENAME TO CREATES PRIVATE HIDEI BIND UNION
%token IMPLEMENTATION TEST REFINES

%token maxParseToken 
%token LEX_WHITE LEX_ERROR LEX_COMMENT




%%
DummyTokensRule
	: {}
	;